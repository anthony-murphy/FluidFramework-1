...
const currentConnectionMode: current.ConnectionMode =
   get_old_ConnectionMode();
...
const currentICapabilities: current.ICapabilities =
   get_old_ICapabilities();
...
const currentIClientDetails: current.IClientDetails =
   get_old_IClientDetails();
...
const currentIClient: current.IClient =
   get_old_IClient();
...
const currentISequencedClient: current.ISequencedClient =
   get_old_ISequencedClient();
...
const currentISignalClient: current.ISignalClient =
   get_old_ISignalClient();
...
const currentIClientJoin: current.IClientJoin =
   get_old_IClientJoin();
...
const currentIProposal: current.IProposal =
   get_old_IProposal();
...
const currentISequencedProposal: current.ISequencedProposal =
   get_old_ISequencedProposal();
...
const currentIApprovedProposal: current.IApprovedProposal =
   get_old_IApprovedProposal();
...
const currentICommittedProposal: current.ICommittedProposal =
   get_old_ICommittedProposal();
...
const currentIPendingProposal: current.IPendingProposal =
   get_old_IPendingProposal();
...
const currentIQuorum: current.IQuorum =
   get_old_IQuorum();
...
const currentIProtocolState: current.IProtocolState =
   get_old_IProtocolState();
...
const currentIProcessMessageResult: current.IProcessMessageResult =
   get_old_IProcessMessageResult();
...
const currentISummaryConfiguration: current.ISummaryConfiguration =
   get_old_ISummaryConfiguration();
...
const currentIClientConfiguration: current.IClientConfiguration =
   get_old_IClientConfiguration();
...
const currentIDocumentMessage: current.IDocumentMessage =
   get_old_IDocumentMessage();
...
const currentIDocumentSystemMessage: current.IDocumentSystemMessage =
   get_old_IDocumentSystemMessage();
...
const currentIBranchOrigin: current.IBranchOrigin =
   get_old_IBranchOrigin();
...
const currentISequencedDocumentMessage: current.ISequencedDocumentMessage =
   get_old_ISequencedDocumentMessage();
...
const currentISignalMessage: current.ISignalMessage =
   get_old_ISignalMessage();
...
const currentIUploadedSummaryDetails: current.IUploadedSummaryDetails =
   get_old_IUploadedSummaryDetails();
...
const currentISummaryContent: current.ISummaryContent =
   get_old_ISummaryContent();
...
const currentIServerError: current.IServerError =
   get_old_IServerError();
...
const currentISummaryProposal: current.ISummaryProposal =
   get_old_ISummaryProposal();
...
const currentISummaryAck: current.ISummaryAck =
   get_old_ISummaryAck();
...
const currentISummaryNack: current.ISummaryNack =
   get_old_ISummaryNack();
...
const currentIHelpMessage: current.IHelpMessage =
   get_old_IHelpMessage();
...
const currentIQueueMessage: current.IQueueMessage =
   get_old_IQueueMessage();
...
const currentINackContent: current.INackContent =
   get_old_INackContent();
...
const currentNackErrorType: current.NackErrorType =
   get_old_NackErrorType();
...
const currentIDocumentAttributes: current.IDocumentAttributes =
   get_old_IDocumentAttributes();
...
const currentIAttachment: current.IAttachment =
   get_old_IAttachment();
...
export const currentICreateBlobResponse: current.ICreateBlobResponse =
   get_old_ICreateBlobResponse();
...
const currentITreeEntry: current.ITreeEntry =
   get_old_ITreeEntry();
...
const currentTreeEntry: current.TreeEntry =
   get_old_TreeEntry();
...
const currentITree: current.ITree =
   get_old_ITree();
...
const currentISnapshotTree: current.ISnapshotTree =
   get_old_ISnapshotTree();
...
const currentISnapshotTreeEx: current.ISnapshotTreeEx =
   get_old_ISnapshotTreeEx();
...
const currentIVersion: current.IVersion =
   get_old_IVersion();
...
const currentSummaryObject: current.SummaryObject =
   get_old_SummaryObject();
...
const currentSummaryTree: current.SummaryTree =
   get_old_SummaryTree();
...
const currentISummaryAuthor: current.ISummaryAuthor =
   get_old_ISummaryAuthor();
...
const currentISummaryCommitter: current.ISummaryCommitter =
   get_old_ISummaryCommitter();
...
const currentSummaryType: current.SummaryType =
   get_old_SummaryType();
...
const currentSummaryTypeNoHandle: current.SummaryTypeNoHandle =
   get_old_SummaryTypeNoHandle();
...
const currentISummaryHandle: current.ISummaryHandle =
   get_old_ISummaryHandle();
...
const currentISummaryBlob: current.ISummaryBlob =
   get_old_ISummaryBlob();
...
const currentISummaryAttachment: current.ISummaryAttachment =
   get_old_ISummaryAttachment();
...
const currentISummaryTree: current.ISummaryTree =
   get_old_ISummaryTree();
...
const currentIUser: current.IUser =
   get_old_IUser();
...
const currentITokenClaims: current.ITokenClaims =
   get_old_ITokenClaims();
...
const currentISummaryTokenClaims: current.ISummaryTokenClaims =
   get_old_ISummaryTokenClaims();
...
const currentIActorClient: current.IActorClient =
   get_old_IActorClient();
...
const currentITokenService: current.ITokenService =
   get_old_ITokenService();
...
const currentITokenProvider: current.ITokenProvider =
   get_old_ITokenProvider();
...
const currentScopeType: current.ScopeType =
   get_old_ScopeType();
...
const currentIConnect: current.IConnect =
   get_old_IConnect();
...
const currentIConnected: current.IConnected =
   get_old_IConnected();
