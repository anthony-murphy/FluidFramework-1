...
export function segmentWalk<TData>(
    ...
    if (startBlock?.children?.[startChild?.index] !== startChild && startBlock.parent === startBlock) {
    ...
    const walked = new Set<ISegment>();
    while (current !== undefined) {
        // go down to the leaf level
        let skip: boolean | undefined = false;
        while (skip !== true && !current.start.isLeaf()) {
            skip = opts.skip?.(current.start);
            current.block = current.start;
            current.start = current.start.children[forward ? 0 : current.start.childCount - 1];
        }
        if (skip !== true) {
            ...
            for (let childIndex = current.start.index;
                ...
                if (opts.skip?.(child) !== true) {
                    assert(child.isLeaf(), "all children must be leaves when start is");
                    assert(!walked.has(child), "not walked");
                    walked.add(child);
                    if (opts.leafAction(child, opts.accum) === false) {
                        return false;
                    }
                }
            }
        ...
        if (current.block.parent !== undefined) {
            // we always need to go up one level from leaves
            current.start = current.block;
            current.block = current.block.parent;

            // walk further up only if current start doesn't have siblings,
            // as we already went up on level from leaves
            // this will only happen with at a boundary index for start
            while (current !== undefined && current.block.children[current.start.index + increment] === undefined) {
                if (current?.block.parent === undefined) {
                    current = undefined;
                } else {
                    current.start = current.block;
            ...
            // the above loop ensured that siblings are possible, or set current undefined
            if (current !== undefined) {
                current.start = current.block.children[current.start.index + increment];
            }
        } else {
            current = undefined;
        }
