...
import { ReferencePosition, ReferenceType } from "..";
import {
                ...
                const validateRefs = (reason: string, workload: () => void)=>{
                    ...
                    for(let c = 1; c < clients.length; c++) {
                        for(let r = 0; r < refs[c].length; r++) {
                            const pos0 = clients[0].getLocalReferencePosition(refs[0][r]);
                            const posC = clients[c].getLocalReferencePosition(refs[c][r]);
                            if(pos0 !== posC) {
                                assert.equal(
                                    pos0, posC,
                ...
                validateRefs("Initialize", ()=>{
                    clients.forEach((c,i)=>{
                        ...
                        for(let t = 0; t < c.getLength(); t++) {
                            ...
                            const lref =
                                c.createLocalReference(
                                    seg.segment!,
                                    seg.offset!,
                                    ReferenceType.SlideOnRemove, {t});
                            refs[i].push(lref);
                        }
                    });
                });
