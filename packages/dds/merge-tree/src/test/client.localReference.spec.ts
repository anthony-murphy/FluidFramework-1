...
describe("MergeTree.Client", () => {
    ...
    it("Remove segments to end with sliding local reference", () => {
        ...
        assert.equal(client1.getLocalReferencePosition(c1LocalRef), client2.getLength() - 1);
    });
    ...
    it("Split segment with no references and append to segment with references", () => {
        ...
        // initialize the local reference collection on the segment, but keep it empty
        {
            ...
            const localRef =
                clients.A.createLocalReference(segment, segInfo.offset!, ReferenceType.Simple, undefined);
        ...
        // add a local reference to the newly inserted segment that caused the split
        {
            ...
            clients.A.createLocalReference(segment, segInfo.offset!, ReferenceType.SlideOnRemove, undefined);
        }
        // apply all the ops
