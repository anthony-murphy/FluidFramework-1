...
describe("MergeTree.Revertibles", () => {
	it("revert insert", () => {
		const clients = createClientsAtInitialState({ initialState: "123" }, "A", "B");
	...
	for (const { name, removeStart, removeEnd, expectedPostRemove } of [
		...
		it(name, () => {
			const clients = createClientsAtInitialState({ initialState: "1-23" }, "A", "B", "C");
			const logger = new TestClientLogger(clients.all);
			let seq = 0;
			const ops: ISequencedDocumentMessage[] = [];
	...
	it("revert two overlapping removes", () => {
		const clients = createClientsAtInitialState({ initialState: "123" }, "A", "B", "C");
		const logger = new TestClientLogger(clients.all);
		let seq = 0;
		const ops: ISequencedDocumentMessage[] = [];
	...
	it("Remove All Original Text and Insert then Revert", () => {
		const clients = createClientsAtInitialState({ initialState: "1-2--" }, "A", "B", "C");
	...
	it("Re-Insert at position 0 in empty string", () => {
		const clients = createClientsAtInitialState({ initialState: "BBC-" }, "A", "B", "C");
	...
	it("Revert Local annotate and remove with intersecting remote annotate", () => {
		const clients = createClientsAtInitialState({ initialState: "1234-----" }, "A", "B", "C");
